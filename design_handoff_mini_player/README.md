...
이 번들의 `mini_player_reference.dc.html` 는 **HTML로 만든 디자인 레퍼런스**다 — 의도한 모양·동작·상태 전환을 보여주는 프로토타입이며, 그대로 가져다 쓰는 프로덕션 코드가 아니다. 할 일은 이 디자인을 **대상 코드베이스의 기존 환경**(이 프로젝트는 TypeScript + 순수 DOM 렌더링 `src/ui/game-ui.ts` 구조)에 맞춰 재구현하는 것이다. 실제 `<audio>`/오디오 엔진, 곡 목록, 기존 사운드 설정(BGM 볼륨/음소거)과 연결해야 한다.

이 파일은 React 클래스(`DCLogic`)로 작성돼 있지만 그건 프로토타입 런타임일 뿐이고, **로직(상태·타이머·포맷팅·이벤트 핸들러)은 프레임워크 무관한 평범한 패턴**이라 그대로 옮기면 된다.
...
- 아이콘 path 는 기존 핸드오프(`handoff/icons.ts`)의 `music / soundOn / soundOff / bee` 와 동일 스타일. 재사용 권장.
- **실제 음원 파일**(BGM 트랙)은 코드베이스 자산으로 별도 준비 필요.

## Files
- `mini_player_reference.dc.html` — 동작하는 HTML 프로토타입(이 폴더). 브라우저로 바로 열어 상태 전환·스크럽·트랜스포트를 확인 가능. 로직 클래스 안에 상태/타이머/핸들러 전체가 들어 있다.
