...
> expert search 는 회랑 밖). expert(빔+depth-4) 대비 self-play **10승 2패(12판)** 로 일반 강도도 우위.
> 학습 네트워크 없이(잘 튜닝된 `evaluate` 를 leaf/롤아웃 + Solver) 풀려, 이 문서가 가정한 "큰 재설계"보다
> 가벼웠다. 단 **완벽하진 않다**(갈색 후공 2패)·느리다(~5s/수). 상세: `ai_strategy.md` "MCTS 엔진(실험)"
> · `session_logs/2026-06-29.md` 세션 2. UI 노출 방식은 미정(속도 트레이드오프).
