...
### 2026-07-08 측정: "후보 시딩" 가설도 기각 — 끊는 수가 후보에 있어도 안 고른다 (마지막 교란 제거)

06-30 그리드의 남은 교란 요인이었던 "회랑 끊는 수가 애초에 MCTS 후보에 없었던 것 아닌가"(06-29
잠금수 교훈의 방어판 재현 가능성, backlog #2 재개 시 1순위 실험)를 측정했다. 프로브:
`probes/_corridor-cut-probe.test.ts`(보존 — 재현 패치 포함).

- **부수 발견(중요)**: `corridorLockThreats` 는 **게임3에서 아예 발화하지 않는다**(전 갈색 차례 위협 0).
  트리거가 "연속 3+ 타일 run"인데, 게임3 노랑은 q=4 줄을 (4,-2)·(4,-3)·(4,-5)처럼 **틈을 둔 채** 유지하다
  twoTiles 한 수로 (4,-4)+(4,-6)을 동시에 메워 **2연속→5연속으로 점프** — 발화할 순간 자체가 없다.
  → **UX '강하게' 코칭도 게임3 유형(틈 회랑)엔 침묵**하는 블라인드 스팟(§6 코칭 폴백의 한계).
- **실험**: 창 기준 넓은 스캔(`corridorWindows`: 5칸 창에 attacker 타일 ≥3 연속 불문 + 방어 0 + 끊을 칸
  ≥1)을 만들어 `mctsActions` **루트에 cutCells 시딩**(강제 아님). 게임3에서 창 스캔은 28·30수(잠금 직전,
  예방 가능한 마지막 두 차례)에 (4,-4)·(4,-6)을 정확히 후보로 올린다.
- **결과: 기각.** 28·30수에서 끊는 수가 후보에 있는데도 기본(sims1000·d2)·sims4000·sims4000+d5 **전부
  기존과 같은 수(회랑 밖)** 를 선택. 게임1 4수 예방은 유지(회귀 없음). → 원인은 후보 부재가 아니라
  **가치 지평 한계**(끊음의 보상 = 10수+ 뒤의 패배 회피라 rollout/leaf 가 못 본다). 06-30 결론이 마지막
  교란까지 제거된 채 확정. 엔진 변경은 되돌림(attackMul 전례와 동일 기준 — 예방 0 = 채택 근거 없음).
- **남는 옵션(미착수)**: ① Stage 3 CNN(위와 동일, 보류). ② **UX 코칭을 창 스캔으로 확장**해 틈 회랑도
  경고 — 단 오발 측정 필요(창 트리거는 보수판보다 자주 발화. 06-30 보수판 오발 10% 이 기준선).
