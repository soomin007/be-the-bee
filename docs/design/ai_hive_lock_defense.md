...
### 2026-06-30 측정: 주도권(공격 배율 attackMul↑)도 무효 — §5 #2 의 단순판 막힘

§5 #2(주도권 강화) 의 가장 단순한 구현, **전문가 공격 배율 `attackMul` 만 올리기**(수비 유지)를 측정.

- **회랑 프로브**(expert + `weights:{attackMul}`): 게임1 4수는 1.0/1.15/1.3 **모두 (-1,1) 예방 유지**
  (망치지 않음). 게임3 32·36수는 1.3 도 **회랑 밖**(개선 0). → 배율↑로 긴 회랑 직접 예방은 안 된다
  (주도권은 회랑 *형성 전* 효과인데 게임3 결정적 수는 이미 형성 중).
- **강도 프록시**(medium 빔 self-play, N=3): attackMul 1.15·1.3 모두 vs easy **100% 유지**(약화 아님)
  지만 거울에서 0-1-5(무승부 잦음, 강화 신호 없음). → **강도 중립**(해롭지도 이롭지도 않음).
- **결론**: 단순 `attackMul↑` 는 채택 근거 없음(직접 예방 0 + 강도 중립). 진짜 주도권은 "초반에 상대
  회랑 형성을 위협으로 응징"하는 더 정교한 메커니즘이 필요한데, 회랑 형성 인식 + 초반 위협은 정적/규칙
  (이미 약화)의 영역이라 막혀 있다. self-play 검증도 한계(AI 끼리는 사람-회랑 정석을 거의 안 둠).

### 종합(2026-06-30): 긴 회랑 예방의 코드 접근은 사실상 다 막혔다
정적 평가(06-27)·규칙 강제(06-28)·MCTS 파라미터(06-30)·주도권 배율(06-30) 모두 무효/약화. 남은 현실적
선택지는 ① **학습 가치함수**(AlphaZero-lite, 큰 작업), 또는 ② AI 강화 대신 **UX 보완**(긴 회랑은 사람
상대 한정 약점이므로, 코칭/경고로 사람에게 "회랑이 잠기기 전에 끊으라"를 안내 — 이미 코칭 인프라 있음).

### value-net 종합(2026-06-30): 학습은 성립하나 blend 는 ship 불가 — 피처-MLP 의 공간 한계가 근본
선택지 ①(학습 가치함수, AlphaZero-lite)을 Stage 0~2 까지 구현·측정했다. **결과: 학습 자체는 성립하나
(Gate 1 통과 — 게임 결과 예측 부호정확도 0.89), 전문가 기본 채택은 불가.** 상세 일지는 `session_logs/
2026-06-30.md` 의 value-net Stage 0-1 / Stage 2 / blend sweet spot 세 섹션.

- **무엇이 됐나**: value-net 단독 leaf(blend 1.0)는 휴리스틱이 못 한 회랑 예방을 학습으로 달성 —
  게임2 4수 키스톤 (3,0)·8수 (4,-1) 선점(off 는 회랑 밖). 코드 접근이 다 못 한 걸 데이터로 부분 달성.
- **왜 ship 불가 (측정 확정, N=5)**: ① blend 1.0(키스톤 잡는 유일한 값)은 vs hard **0-6** 완패(일반
  전술 약함). ② 강도를 회복하려 휴리스틱과 blend 하면(0.4~0.6) 회랑 catch 가 무너진다 — 게임2 4수
  키스톤은 **0.95 에서 이미 뒤집혀** 1.0 미만 전부 놓침(value-net 의 키스톤 선호 마진이 면도날). ③ 그래도
  남는 회랑 이득(게임2 8수)을 가진 blend 0.4~0.6 조차 vs hard **5~6/10** 으로 **off(10-0) 대비 큰 강도
  회귀**. 즉 (회랑 잡는 blend = 강도 완패) ↔ (강도 회복 blend = 회랑 놓침) 의 trade-off 가 풀리지 않는다.
- **근본 원인**: `encodeFeatures` 는 21개 **전역 스칼라** 요약이라 "어느 빈 칸이 급소인가"의 공간 정보가
  없다. 키스톤 (3,0) vs 대안 (0,1) 의 *결과 보드* 피처 차가 미미해 value-net 선호가 약하고(blend 에 쉽게
  희석), 동시에 전역 요약으론 일반 전술 정밀도가 휴리스틱+rollout 에 못 미친다(강도 회귀). 게임3(긴 회랑)
  미달과 **같은 뿌리** — 공간 인코딩 부재.
- **남은 길**: ① **Stage 3**(radius-7+ 재중심 래스터 + 소형 CNN, Python 학습 → weights→ts, TS 추론 불변)
  만이 공간 한계를 넘을 수 있으나, **강도 회귀 신호 때문에 ROI 불확실**(CNN 도 일반 전술이 휴리스틱보다
  약하면 같은 trade-off 재발 가능 — 큰 투자 전 신중히). ② **UX 코칭 폴백**(가장 실용적): 긴 회랑은 사람
  상대 한정 약점이고 코칭 인프라가 이미 있으니, 사람에게 "벌집 줄이 잠기기 전에 그 위를 막아라"를 안내.
- **현 상태가 가장 강한 AI**: 기본 expert = MCTS(rollout+휴리스틱 leaf)가 vs hard 10-0. value-net 은 옵션
  (`valueNet`/`valueNetBlend`)으로 보존하되 기본 off. (known_issues 2026-06-29 "MCTS 실험·기본 off" 는
  낡음 — 그 뒤 expert 기본이 MCTS 로 전환됐다.)
