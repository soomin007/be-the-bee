...
| 2 | (사용자) BGM 파일 | Suno로 생성 → `public/bgm.mp3` (프롬프트: `bgm_prompt.md`) |
| 3 | 저장/불러오기·리플레이 | 엔진 직렬화 가능 → localStorage 저장, 기보 재생 |
| 4 | **멀티플레이 로비**(미래) | 초대링크 방 + 로비에서 RoomSettings 공통 적용. 엔진 직렬화 준비됨. 네트워킹은 아직 안 만듦 |
